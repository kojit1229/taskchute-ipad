...
// (c) health/insightsのconsumer文字列と、FUNDの実モジュールが組み立てるGET経路を突合する。
...
// consumerSnippet は既存health/insightsのコード断片。consumerProbe は同期の隔離子プロセスで
// 実FUND経路を実行し、合成接続と偽fetchだけで確認する。追加3ファイルは未作成を許容する。
const CONTRACT = [
  {
  ...
  },
  {
    ...
    consumer: "src/features/fund.js → workspace → read-gateway → read-transport",
    consumerProbe: "fund",
  ...
  ...["fund-codex", "fund-codex-status", "fund-comparison"].map(name => ({
    resolvedPath: `taskchute/dashboard/${name}.json`, consumer: "FUND captured read transport",
    consumerProbe: "fund", optionalFile: true, writerScript: null, requiredPatterns: []
  }))
];
...
function runGate(contract, loopDir, personalDataDir, sourceRoot = ROOT, log = console.log) {
  ...
  for (const entry of contract) {
    ...
    if (personalDataCloned) {
      ...
      } else if (entry.optionalFile) {
        log(`  -   (b) 未作成を許容する追加データ: ${entry.resolvedPath}`);
      } else {
        failures++;
    ...
    if (entry.consumerProbe === "fund") {
      if (!fundProbe) fundProbe = spawnSync(process.execPath, ["--experimental-vm-modules",
        path.join(__dirname, "fund-contract-probe.js"), sourceRoot], { encoding: "utf8", timeout: 15000, windowsHide: true });
      let result;
      try { result = JSON.parse(fundProbe.stdout); } catch { result = null; }
      if (fundProbe.status === 0 && !fundProbe.error && result?.ok === true && result.paths?.includes(entry.resolvedPath)) {
        log("  OK  (c) 合成接続で実モジュールのGET経路とtaskchute前置を確認");
      } else {
        failures++; log("  NG (c) FUND実読み取り経路が不一致・欠落・実行不能");
      }
    } else if (entry.consumerProbe === undefined && typeof entry.consumerSnippet === "string"
      && entry.consumerSnippet.trim().length > 0 && consumerSource.includes(entry.consumerSnippet)) {
