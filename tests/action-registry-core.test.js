...
const GOLDEN_CLICK_ACTIONS = [
  ...
  "draft-leave-save", "draft-leave-stay", "draft-leave-discard",
  "lev-judge",
...
const APP_JS_REGISTERED_ACTIONS = [
  ...
  "today-add-interruption", "today-add-actual", "remaining-shift", "remaining-tomorrow",
...
function extractAppRegisteredActions() {
  ...
  const names = [];
  const tree = acorn.parse(appSource, { ecmaVersion: "latest", sourceType: "module" });
  const owners = tree.body.filter(n => n.type === "ExpressionStatement" && n.expression.callee?.name === "configureInstruments");
  if (owners.length !== 1) throw new Error("one configureInstruments owner required");
  const callbacks = owners[0].expression.arguments[0].properties.filter(p => p.key?.name === "registerActions");
  if (callbacks.length !== 1) throw new Error("one instruments registration callback required");
  const callback = callbacks[0].value;
  const expected = acorn.parse('(handlers) => registerActions({ ...handlers, "instruments-open-iron-log": () => setView("iron-log") })', {ecmaVersion:"latest"}).body[0].expression;
  const shape = n => JSON.stringify(n, (k,v) => ["start","end","raw"].includes(k) ? undefined : v);
  if (shape(callback) !== shape(expected)) throw new Error("instruments callback structure changed");
  const forwarded = callback.body;
  function visit(node) {
    if (!node || typeof node !== "object") return;
    if (node.type === "CallExpression" && node.callee.name === "registerActions") {
      const object = node.arguments[0];
      if (!object || object.type !== "ObjectExpression") throw new Error("registerActions must use an object literal");
      for (const property of object.properties) {
        if (property.type === "SpreadElement") {
          if (node !== forwarded) throw new Error("unexpected action spread");
          continue; // preserve existing forwarding; explicit override is counted below
        }
        if (property.computed || typeof property.key.value !== "string") throw new Error("action registration must have a literal name");
...
(async () => {
  ...
  console.log("[3-b] 5feature(wish/journal/timeline/today/track-ui)のconfigureXxxを空depsで呼び"
    + "+fund.js(実configureでbridgeを構築)を合わせ、レジストリに実際に登録される"
  ...
  // twelve-week.jsはimport時点で登録済み。FUNDは実configure呼出しで登録する。
  // Registration now occurs in configureFund; exercise the actual facade/bridge.
  fundMod.configureFund({ root: { addEventListener() {} },
    escapeHTML: String, renderMarkdown: String, renderHeader: () => "",
    personalDataReady: () => false,
    personalDataConn: () => { throw new Error("registration must not read connection"); },
    githubHeaders: () => { throw new Error("registration must not fetch"); }
  });
  void twelveWeekMod;
  ...
  const submitBody = appSource.slice(appSource.indexOf("function submitModal(options) {"), appSource.indexOf("function modalDeleteMessage("));
  check("submitModalはtype/idをsnapshotし、詳細transactionとその他の両方で同じfieldsをdispatcherへ渡す",
    submitBody.includes("const { type, id } = state.modal;")
    && submitBody.includes('if (["task", "project", "block"].includes(type))')
    && submitBody.includes("return draftSaveTransaction.run(() => dispatchModalSave(type, id, fields), options);")
