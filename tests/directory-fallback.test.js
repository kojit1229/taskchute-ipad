...
// 固定時計: 一覧の生成時刻(2026-09-06)から48時間以内に固定し、実行日が進んでも stale 判定へ落ちないようにする。呼ぶたびに1ms進めて lastSuccessAt 等の比較を有効に保つ。
const clock=()=>{let t=Date.parse('2026-09-06T12:00:00Z');return ()=>t++;};
const gateway=options=>createFundReportGateway({now:clock(),...options});
test('initial valid index avoids directory, force unions while explicit poison metadata wins',async()=>{
 let dirs=0;const g=gateway({captureConnection,read:async()=>index([{name,kind:'WRONG',date:'2026-09-05'}]),readDirectory:async o=>{dirs++;assert.equal(o.context,'captured');return ok([{name,type:'file'},{name:other,type:'file'},{name:'monthly.md',type:'file'}]);}});
...
test('missing broken and throwing index fall back to directory without invented generation timestamp',async()=>{
 ...
 const g=gateway({captureConnection,read,readDirectory:async()=>ok([{name,type:'file'},{name:'folder',type:'dir'}])});
...
test('partial force success retains valid index and reports directory failure; both failures keep lastGood',async()=>{
 let good=true;const g=gateway({captureConnection,read:async()=>good?index([{name}]):{ok:false,status:500},readDirectory:async()=>ok({not:'a directory'})});
...
test('connection changes during directory discard late result and abort captured signal',async()=>{
 let revision=1,release,signal;const g=gateway({captureConnection:()=>({ready:true,revision,readContext:revision}),read:async()=>({ok:false,status:404}),readDirectory:async o=>{signal=o.signal;return await new Promise(r=>release=r);}});
...
test('one deadline bounds directory and late result cannot overwrite retry',async()=>{
 let hang=false,release;const g=gateway({captureConnection,timeoutMs:15,read:async()=>({ok:false,status:404}),readDirectory:async()=>hang?await new Promise(r=>release=r):ok([{name,type:'file'}])});
...
test('optional directory absence keeps original index errors and markdown never calls directory',async()=>{
 const g=gateway({captureConnection,read:async()=>({ok:false,status:404})});assert.equal((await g.load('report-index.json')).state,'missing');
 let dirs=0;const h=gateway({captureConnection,read:async()=>({ok:false,status:404}),readDirectory:async()=>{dirs++;return ok([]);}});
