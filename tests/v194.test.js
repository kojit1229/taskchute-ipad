...
(async () => {
  ...
  async function seedWbs(parent, children) {
    ...
    await page.waitForFunction((ids) => {
      ...
      const renderedIds = new Set(Array.from(document.querySelectorAll('span[data-action="edit-task"][data-id]'), (el) => el.dataset.id));
      return ids.every((id) => renderedIds.has(id));
    }, tasks.map((task) => task.id));
  }

  async function wbsSiblingOrder(ids) {
    return page.locator('span[data-action="edit-task"][data-id]').evaluateAll((elements, wantedIds) => {
