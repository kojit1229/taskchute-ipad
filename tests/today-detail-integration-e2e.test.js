 ...
 try {
  ...
  for (const name of ['A', 'B', 'C', 'D'].filter(name => !which || which === name)) {
   ...
   try {
    ...
    await page.reload();
     if (name === 'A') await page.locator('[data-action="wbs-select-project"][data-id="fixture-project"]').click();
     await page.locator(name === 'A' ? '[data-work-list="wbs-tasks-fixture-project"]' : '[data-work-list="exec"]').waitFor();
    ...
    if (name === 'A') {
     const root = page.locator('[data-work-list="wbs-tasks-fixture-project"]'), rows = root.locator('[data-work-list-rows]'), query = root.locator('[data-work-filter="query"]');
     ...
     const last = rows.locator('[data-work-key="task:task-299"] button.wbs-task-title[data-action="edit-task"]');
     ...
     await page.evaluate(() => { window.__scanOpen = 0; let had = false; new MutationObserver(() => { const has = Boolean(document.querySelector('#bodyScanComment')); if (has && !had) window.__scanOpen++; had = has; }).observe(document.querySelector('#modalRoot'), {childList:true,subtree:true}); });
      assert.equal(await page.locator('#modalRoot details:not([open])').count(), 0);
      assert(await page.locator('#modalRoot [data-action="toggle-task-complete"]').isVisible());
      await page.locator('#modalRoot [data-action="toggle-task-complete"]').click();
      assert.equal((await read()).tasks.find(row => row.id === 'task-299').status, 'todo');
      assert.equal((await read()).blocks.find(row => row.id === 'block').comment, '元Blockメモ');
      assert.equal(await page.locator('#bodyScanComment').count(), 0);
     ...
     await page.locator('[data-work-list="wbs-projects"]').waitFor();
      await page.locator('[data-action="wbs-select-project"][data-id="fixture-project"]').click();
     ...
     assert((await page.locator('[data-work-list="exec"] h2').textContent()).includes(PREV));
     assert.deepEqual(await page.locator('[data-work-list="exec"] [data-work-key]').evaluateAll(rows => rows.map(row => row.dataset.workKey)), ['block:previous']);
     assert.equal(await page.locator('[data-work-list="exec"] [data-work-key="block:previous"] [data-action="now-start"]').count(), 0);
