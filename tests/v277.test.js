...
(async () => {
  ...
  try {
    ...
    console.log("[3] UI-A Todayは1280px境界で予定/記録/ジャーナルの3列へ切替");
    for (const width of [1279, 1280, 1440, 1920]) {
      ...
      if (width < 1280) {
        ...
        check(`${width}px Todayは予定/記録/ジャーナルの比率1.8:1:1.4の3列`, today.columns.length === 3 && total > 0
          && today.columns.every((value, index) => Math.abs(value / total - [1.8 / 4.2, 1 / 4.2, 1.4 / 4.2][index]) < 0.002), JSON.stringify(today));
      }
    }
