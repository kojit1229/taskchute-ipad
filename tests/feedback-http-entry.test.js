...
test('actual wiring owns journal DOM, reflects current draft, uses actual sanitizer and preserves four invalidations',()=>{
 ...
 const c={state,_imeComposing:false,document:{querySelector:()=>element},isArchivedDate:()=>false,draftSaveTransaction:{active:false},draftLeaveGuard:{active:false},nowDateTime:()=> 'fixture stamp',ensureJournal(){},structuredClone,
  ...
  // v400 ハーネス追随(監督者 2026-09-13、束R2-D 2回-05): sanitizedStateForGitHub が系列の容器検査 validateScheduleContainers も呼ぶ。同じ型の代役(製品変更なし)。
  validateScheduleContainers(snapshot){c.validateSingleScheduleContainer(snapshot.singleSchedules);if(snapshot.scheduleSeries!==undefined&&!Array.isArray(snapshot.scheduleSeries))throw Error('fixture: schedule-series container');}};
