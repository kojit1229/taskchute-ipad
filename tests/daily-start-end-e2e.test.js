  ...
  try {
    ...
    for (const action of ['now-end', 'daily-block-end']) {
      ...
      await page.evaluate(key => {
        ...
        Storage.prototype.setItem = function(k,v) {
          if (k === key) { window.__endWrites++; if (window.__endWrites === 2) window.__endReportWrite = JSON.parse(v); if (window.__endFail) throw new DOMException('fixture quota', 'QuotaExceededError'); }
      ...
      // v386 契約追随(監督者決定 2026-09-11、design/03 §5、fixB6)
       ...
       const successfulWrite = await page.evaluate(() => window.__endReportWrite);
       assert(successfulWrite.reports[saved.blocks[0].date].includes(saved.blocks[0].title),
          JSON.stringify({ action, block: saved.blocks[0], report: successfulWrite.reports[saved.blocks[0].date] }));
       assert(successfulWrite.reports[saved.blocks[0].date].includes(saved.blocks[0].id));
