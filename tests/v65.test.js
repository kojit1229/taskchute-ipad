...
(async () => {
  ...
  try {
    ...
    // fixV393: 常設欄で同じ選択・保存を検査する。
    check("レバレッジselectは常設で表示済み", await page.locator('[data-modal-field="leverageType"]').isVisible()
      && await page.locator('[data-modal-field="leverageType"]').evaluate(el => !el.closest('details:not([open])')));
