...
// (c) 2件目の抑制: 実験中に「別の実験を試したい」/フィードバック欄の「🧪 実験にする」を押しても
//     モーダルは開かず「1つに絞る」文言のトーストが出る。
...
(async () => {
  ...
  try {
    ...
    console.log("[7] ジャーナルのAIフィードバック欄「🧪 実験にする」ボタンも同じガードがかかる");
    check("フィードバック欄に🧪実験にするボタンがある", await page.locator('[data-action="experiment-add"]:has-text("実験にする")').count() === 1);
    await page.click('[data-action="experiment-add"]:has-text("実験にする")');
    await page.waitForTimeout(300);
    check("実験中はここからもモーダルが開かない", await page.locator(".modal-card").count() === 0);
