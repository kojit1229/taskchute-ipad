...
function parseStamp(value) {
  ...
  const match = typeof value === "string" && value.length === 19 && STAMP_PATTERN.exec(value);
  if (!match) throw new TypeError("Mutation stamp must use YYYY-MM-DDTHH:mm:ss");
  ...
  if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1
    ...
    throw new TypeError("Mutation stamp contains an invalid calendar date or time");
  }
  return date.getTime();
}
 ...
 * The real clock is required; empty candidate stamps are ignored.
 * Nonempty candidates must be an array of second-precision clock strings.
 * Invalid stamps throw TypeError even when another candidate is newer.
 */
